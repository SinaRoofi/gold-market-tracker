...
# ═══════════════════════════════════════════════════════
# انتخاب نسخه: 'plotly' یا 'matplotlib'
# ═══════════════════════════════════════════════════════
TREEMAP_VERSION = 'matplotlib'  # ← اینجا رو تغییر بده
...
def send_to_telegram(bot_token, chat_id, data, dollar_prices, gold_price, gold_yesterday, gold_time, yesterday_close):
    ...
    try:
        ...
        if TREEMAP_VERSION == 'matplotlib':
            img1_bytes = create_combined_image_matplotlib(data["Fund_df"], dollar_prices["last_trade"], gold_price, gold_yesterday, data["dfp"], yesterday_close)
        else:
            img1_bytes = create_combined_image_plotly(data["Fund_df"], dollar_prices["last_trade"], gold_price, gold_yesterday, data["dfp"], yesterday_close)
...
def create_combined_image_matplotlib(Fund_df, last_trade, Gold, Gold_yesterday, dfp, yesterday_close):
    """تولید تصویر با matplotlib + squarify"""
    ...
    # فونت فارسی
    try:
        font_path = "assets/fonts/Vazirmatn.ttf"
    ...
    squarify.plot(sizes=sizes, label=None, color=colors, alpha=0.9, edgecolor='#1A1A1A', linewidth=3, ax=ax1, text_kwargs={'fontsize': 0})
    ax1.axis('off')
    ...
    for rect, (idx, row) in zip(rects, df_sorted.iterrows()):
        ...
        if area > 0.15:
            ax1.text(x, y + height * 0.12, name, ha='center', va='center', fontproperties=prop_large, color='white')
            ax1.text(x, y, f"{price} {change}", ha='center', va='center', fontproperties=prop_medium, color='white')
            ax1.text(x, y - height * 0.12, bubble, ha='center', va='center', fontproperties=prop_small, color='white', alpha=0.95)
        elif area > 0.08:
            ax1.text(x, y + height * 0.1, name, ha='center', va='center', fontproperties=prop_medium, color='white', weight='bold')
            ax1.text(x, y - height * 0.08, f"{price} {change}", ha='center', va='center', fontproperties=prop_small, color='white')
        elif area > 0.03:
            ax1.text(x, y, name, ha='center', va='center', fontproperties=prop_small, color='white', weight='bold')
    
    ax1.text(0.5, 1.02, '📊 نقشه بازار ۱۰ صندوق طلا با ارزش معاملات بالا', transform=ax1.transAxes, ha='center', va='bottom', fontproperties=prop_title, color='#FFD700')
    ...
    table_data = [[idx, f"{row['close_price']:,.0f}", f"{row['NAV']:,.0f}", f"{row['close_price_change_percent']:+.2f}%", 
                   f"{row['nominal_bubble']:+.2f}%", f"{row['ekhtelaf_sarane']:+.2f}", f"{row['pol_hagigi']:+,.0f}", f"{row['value']:,.0f}"]
                  for idx, row in top_10.iterrows()]
    
    table = ax2.table(cellText=table_data, colLabels=["نماد", "قیمت", "NAV", "تغییر %", "حباب %", "اختلاف سرانه", "پول حقیقی", "ارزش معاملات"],
    ...
    for i in range(len(table_data) + 1):
        for j in range(8):
            ...
            if i == 0:
                ...
                if j in [3, 4, 5, 6]:
                    try:
                        val = float(table_data[i-1][j].replace('%', '').replace('+', '').replace(',', ''))
                        if val > 0:
    ...
    try:
        font = ImageFont.truetype("assets/fonts/Vazirmatn.ttf", 70)
    except:
        try:
            font = ImageFont.truetype("Vazirmatn.ttf", 70)
        except:
...
def create_combined_image_plotly(Fund_df, last_trade, Gold, Gold_yesterday, dfp, yesterday_close):
    """نسخه Plotly"""
    fig = make_subplots(rows=2, cols=1, row_heights=[0.65, 0.35], vertical_spacing=0.02, specs=[[{"type": "treemap"}], [{"type": "table"}]])
    df_sorted = Fund_df.copy()
    df_sorted["color_value"] = df_sorted["close_price_change_percent"]
    df_sorted["display_text"] = df_sorted.apply(lambda r: f"{r.name}\n{r['close_price']:,.0f} ({r['close_price_change_percent']:+.2f}%)\n{r['nominal_bubble']:+.2f}%", axis=1)
    df_sorted = df_sorted.sort_values("value", ascending=False)
    
    colorscale = [[0.0, "#E57373"], [0.1, "#D85C5C"], [0.2, "#C94444"], [0.3, "#A52A2A"], [0.4, "#6B1A1A"], [0.5, "#2C2C2C"],
                  [0.6, "#1B5E20"], [0.7, "#2E7D32"], [0.8, "#43A047"], [0.9, "#5CB860"], [1.0, "#66BB6A"]]
    
    fig.add_trace(go.Treemap(labels=df_sorted.index, parents=[""] * len(df_sorted), values=df_sorted["value"], text=df_sorted["display_text"],
                             textinfo="text", textposition="middle center", textfont=dict(size=14, family="Arial", color="white"), hoverinfo="skip",
                             marker=dict(colors=df_sorted["color_value"], colorscale=colorscale, cmid=0, cmin=-10, cmax=10, line=dict(width=3, color="#1A1A1A")),
                             pathbar=dict(visible=False)), row=1, col=1)
    
    top_10 = df_sorted.head(10)
    table_cells = [top_10.index.tolist(), [f"{x:,.0f}" for x in top_10["close_price"]], [f"{x:,.0f}" for x in top_10["NAV"]],
                   [f"{x:+.2f}%" for x in top_10["close_price_change_percent"]], [f"{x:+.2f}%" for x in top_10["nominal_bubble"]],
                   [f"{x:+.2f}" for x in top_10["ekhtelaf_sarane"]], [f"{x:+,.0f}" for x in top_10["pol_hagigi"]], [f"{x:,.0f}" for x in top_10["value"]]]
    ...
    cell_colors = [["#1C2733"] * len(top_10)] * 3 + [[col_color(x) for x in table_cells[i]] for i in [3, 4, 5, 6]] + [["#1C2733"] * len(top_10)]
    
    fig.add_trace(go.Table(header=dict(values=[f"<b>{h}</b>" for h in ["نماد", "قیمت", "NAV", "تغییر %", "حباب %", "اختلاف سرانه", "پول حقیقی", "ارزش معاملات"]],
                                        fill_color="#242F3D", align="center", font=dict(color="white", size=17, family="Vazirmatn, Arial"), height=32),
                           cells=dict(values=table_cells, fill_color=cell_colors, align="center", font=dict(color="white", size=17, family="Vazirmatn, Arial"), height=35)),
                  row=2, col=1)
    
    fig.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000", height=1400, width=1400, margin=dict(t=90, l=10, r=10, b=10),
                      title=dict(text="<b>📊 نقشه بازار ۱۰ صندوق طلا با ارزش معاملات بالا </b>", font=dict(size=32, color="#FFD700", family="Vazirmatn, Arial"),
                                 x=0.5, y=1.0, xanchor="center", yanchor="top"), showlegend=False)
    ...
    draw = ImageDraw.Draw(watermark_layer)
    try:
        font = ImageFont.truetype("assets/fonts/Vazirmatn.ttf", 60)
    except:
        try:
            font = ImageFont.truetype("Vazirmatn.ttf", 60)
        except:
            font = ImageFont.load_default()
